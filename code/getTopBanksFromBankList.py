...
def doTask(bank_ids: str, sort_by: str, limit: int):
    import json
    bank_id_list = list(map(int, bank_ids.split('|')))
    top_banks = get_top_n_banks_from_list(bank_ids=bank_id_list, sort_by=sort_by, sort_order='desc', limit=limit)
    top_bank_ids = [bank['ID'] for bank in top_banks if 'ID' in bank]
    general_info = get_general_and_financial_bank_info_from_list(top_bank_ids)
