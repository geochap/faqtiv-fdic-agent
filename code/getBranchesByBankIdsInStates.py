...
def doTask(bank_ids: str, state_codes: str):
    import json
    bank_id_list = list(map(int, bank_ids.split('|')))
    state_code_list = state_codes.split('|')
    banks_in_states = get_banks_with_branches_in_list_of_states(state_code_list)
    filtered_bank_ids = [bank['ID'] for bank in banks_in_states if bank['ID'] in bank_id_list]
    branches_info = get_bank_branches(filtered_bank_ids)
