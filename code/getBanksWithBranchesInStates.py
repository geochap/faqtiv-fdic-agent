...
def doTask(state_codes: str):
    import json
    state_code_list = state_codes.split('|')
    banks = get_banks_with_branches_in_list_of_states(state_code_list)
    bank_ids = [bank['ID'] for bank in banks if 'ID' in bank]
    general_info = get_general_and_financial_bank_info_from_list(bank_ids)
